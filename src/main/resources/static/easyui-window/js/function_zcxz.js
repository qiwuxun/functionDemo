...
function openCombotree(ID) {
    ...
    window.setTimeout(function(){
        $box.next().find('span').html('<a style="cursor:pointer;" id="' + btnID + '"><img src="/static/easyui-window/images/code.gif" style="margin-top:1px;"/></a>');
        //初始化按钮
            ...
            openDivForm({
            ...
            }, [
                {
                    ...
                    handler: function () {
                        ...
                        //单位控制人员
                        var myDictUrl = options.url;
                        var myIndex = myDictUrl.indexOf("=");
                        var myCodeType = myDictUrl.substring(myIndex,myDictUrl.length);
                        if(myCodeType=='=CODE_UNIT'||myCodeType=='=CODE_XZUNIT'||myCodeType==='=CODE_QGUNIT'){
                            showRy(dwmc);
                        }
